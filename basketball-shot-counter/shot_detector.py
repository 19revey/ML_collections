...
class ShotDetector:
    def __init__(self):
        ...
        # self.model = YOLO("runs/detect/train2/weights/best.pt") #100 epoch small dataset
        self.model = YOLO("runs/detect/train5/weights/best.pt") #10 epoch large dataset
        ...
        self.cap = cv2.VideoCapture("/app/test.mp4")
        # youtube_url = 'https://www.youtube.com/watch?v=c7n8G4wK2Ss&ab_channel=GoldenStateWarriors'
        # youtube_url = 'https://www.youtube.com/watch?v=5zPxyWLls98&ab_channel=JohnPaulsen'
        
        # self.cap = cap_from_youtube(youtube_url, '720p')
    ...
    def drawdetectarea(self):
        if len(self.hoop_pos) > 0 and len(self.ball_pos) > 0:
            x1 = self.hoop_pos[-1][0][0] - 3 * self.hoop_pos[-1][2]
            x2 = self.hoop_pos[-1][0][0] + 3 * self.hoop_pos[-1][2]
            # y1 = self.hoop_pos[-1][0][1] - 2 * self.hoop_pos[-1][3]
            y1=0
            y2 = self.hoop_pos[-1][0][1] - int(0.5 * self.hoop_pos[-1][3])
            w=x2-x1
            h=y2 -y1
        ...
        while True:
            ...
            for r in results:
                ...
                for box in boxes:
                    ...
                    # if cls==1:
                    #     logging.info('basketball detection at frame {} conf {}'.format(self.frame_count,r[0].boxes.conf))

                    center = (int(x1 + w / 2), int(y1 + h / 2))

                    # Only create ball points if high confidence or near hoop
                    if (conf > .4 or (in_hoop_region(center, self.hoop_pos) and conf > 0.4)) and current_class == "Basketball":
                        self.ball_pos.append((center, self.frame_count, w, h, conf))
                        cvzone.cornerRect(self.frame, (x1, y1, w, h),colorC=(0,0,255))
